...
\begin{figure}[t]
    ...
    \caption{Overview of user-metric monitoring.
        Before monitoring, each microservice registers the physical address and size of its metric structs in a per-process metrics descriptor array.
...
Furthermore, \texttt{XMON\_IOC\_SET\_ELEM} pins the pages of the registered memory region so that they are never swapped out (See Sec.~\ref{subsec:non-blocking}).
This prevents page faults and blocking caused by swap-in during monitoring.
